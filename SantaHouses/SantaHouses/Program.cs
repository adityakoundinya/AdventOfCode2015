...
namespace SantaHouses
{
    class Program
    {
        static Dictionary<string, int> houseCount = new Dictionary<string, int>();
        ...
        static void Main(string [] args)
        {
            ...
            foreach (char i in input)
            {
                if (santa)
                {
                    NavigateHouses(i, ref santaRow, ref santaColumn);
                }
                else
                {
                    NavigateHouses(i, ref roboRow, ref roboColumn);
                }
                santa = !santa;
            }
            Console.WriteLine("Houses: " + houseCount.Count);
            Console.Read();
        }

        private static void NavigateHouses(char i, ref int row, ref int column)
        {
            switch (i)
            {
                case '^':
        ...
        static void AddHouse(int x, int y)
        {
            string key = x.ToString() + "," + y.ToString();
            if (houseCount.ContainsKey(key))
            {
                houseCount [key] = houseCount [key]++;
            }
            else
            {
                houseCount.Add(key, 1);
            }
        }
    }
}
