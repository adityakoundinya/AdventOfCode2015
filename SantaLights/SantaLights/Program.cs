...
namespace SantaLights
{
    class Program
    {
        ...
        static void Main(string [] args)
        {
            ...
            List<string> rounds = input.Split(',').ToList();
            foreach (string i in rounds)
            {
                string [] commands = i.Split(' ');
                if (commands.Length == 5)
                {
                    ExecuteLights(commands [2], commands [4], commands [0] + commands [1]);
                }
                else if (commands.Length == 4)
                {
                    ExecuteLights(commands [1], commands [3], commands [0]);
                }
            }
            Console.WriteLine("Lights On: " + CountOnLights().ToString());
            Console.Read();
        }

        static void ExecuteLights(string start, string end, string command)
        {
            string [] startingPoint = start.Split('-');
            string [] endPoint = end.Split('-');

            int x = int.Parse(startingPoint [0]);
            int y = int.Parse(startingPoint [1]);

            int endX = int.Parse(endPoint [0]);
            int endY = int.Parse(endPoint [1]);
